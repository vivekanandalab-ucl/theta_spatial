...
folder          = 'C:\Users\uvivekananda\Documents\MATLAB\spatial orientation\New Cue Period Data\';
folder2         = 'C:\Users\uvivekananda\Documents\MATLAB\spatial orientation\DanSpikeGUI\';
%patientNos          = [2 4 5 6 8 9 10 12 13 14 15 16 19];
patientNos          = [2 5 6 8 9 10 13 14 15];                                  % List of patient IDs to analyse
cueWin              = [5 8];                                            % Time window for the analysis
region              = 'RealHPC';                                                % Region to be analysed
fOrder              = 2;                                                        % Filter order (use 2 as a default)
freqBands           = [30:1:50,50:2:100,100:5:120];
phaseBand           = [2 12];                                                    % Frequency band for phase (i.e. theta band)
...
phaseBins           = 20;                                                       % Number of phase bins (use 30 as a default)
downFs              = 512;
...
for p               = 1 : length(patientNos)
    ...
    for c               = 1:length(channels) % ADD FOR C = 1 : LENGTH(CHANNELS)
        ...
        % Filter the data in the high frequency band
